...
const textConfig: {[key:string]: string} = {
  ...
  "create-account-and-login":
    "I will use Dappier's create-account-and-login tool to create account and login myself using the KYA token from Skyfire",
  "search-dataset":
  ...
  if (!agentContext || Object.keys(agentContext).length === 0) {
    agentContext = {
      available_mcp_servers: [
        ...
        { url: process.env.DAPPIER_KEYCLOAK_MCP_URL || "", headers: {} },
      ...
      conversation_history: [
        {
          role: "system",
          content:
            "You are an e-commerce buyer agent assisting user to make a purchase. When connect-mcp-server-tool tool is executed, you must stop agent execution immediately-- there should be no subsequent tools executed. Do not truncate or mutate tokens. Use the Keycloak version of the service."
        },
      ],
    };
  }
