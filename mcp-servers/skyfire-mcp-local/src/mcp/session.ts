...
export async function handleMcpMessageUnsafe(
  ...
  if (isInitRequest) {
    const apiKey = req.headers['skyfire-api-key']
    const authorization = req.headers.authorization
    if (apiKey === undefined && authorization === undefined) {
      res
        .status(401)
        .header(
          'www-authenticate',
    ...
    if (apiKey === undefined) {
      if (authorization !== undefined) {
        // Pseudo code - placeholder if we adopt mcp oauth proposed standard
        // validate token
        // look up token in token hash cache
        //   if token hash found, look up session id,
        //      if session id found, return existing session id
        //      if session id not found (rare), return conflict
        //   if token hash not found
        //      create an API key (use expiration)
        //      store the API key in the token hash cache
        //      continue with the code as below with the api key
        //
      }
    }
    ...
    const mcpServer = new SkyfireMCP(apiKey)
